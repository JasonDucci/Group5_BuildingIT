...
function EditArt() {
  ...
  useEffect(() => {
    if (art?._id !== id) {
      ...
      setValue("year", art?.year);
      setValue("artist", art?.artist);
      setValue("medium", art?.medium);
      setValue("category", art?.category);
      setValue("desc", art?.desc);
      setImageWithoutTitle(art?.image);
      setImageTitle(art?.titleImage);
    }
    if (isSuccess) {
  ...
  }, [dispatch, id, art, setValue, isSuccess, editError, navigate]);
    ...
    <SideBar>
      {isLoading ? (
        ...
        <div className="flex flex-col gap-6">
          <h2 className="text-xl font-bold">Update "{art?.name}"</h2>
