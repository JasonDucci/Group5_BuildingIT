...
function CategoryModal({ modalOpen, setModalOpen, category }) {
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    if (title) {
      //if cate is available then update it else create new one
      if (category) {
      ...
      <div className="inline-block sm:w-4/5 border border-border md:w-3/5 lg:w-2/5 w-full align-middle p-10 overflow-y-auto h-full bg-main text-white rounded-2xl">
        ...
        >
          ...
          <button
            ...
            type="submit"
            className="w-full flex-colo py-3 text-lg transitions hover:bg-dry border-2 border-subMain rounded bg-subMain text-white"
          >
            {isLoading || upLoading
              ? "Loading..."
              : category
